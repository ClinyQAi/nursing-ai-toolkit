---
sidebar_position: 1
title: Overview
description: Real-world examples of Generative AI in nursing education
---

![Nursing Case Studies](@site/static/img/cover_case_studies.png)

# Nursing Case Studies

:::info Attribution
**Original work**: "Educators' guide to multimodal learning and Generative AI" — Tünde Varga-Atkins, Samuel Saunders, et al. (2024/25) — [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/)  
**Adapted for UK Nursing Education by**: Lincoln Gombedza, RN (LD)  
**Last Updated**: December 2025
:::

Explore how Generative AI is being applied across various nursing education contexts. These case studies provide practical insights, student feedback, and direct alignment with NMC standards.

## Case Study Categories

We have organized our examples into several key domains of nursing practice:

### 🩺 [Patient Scenarios](./patient-scenarios/index.md)
Practice history taking, NEWS2 assessment, and clinical reasoning with virtual patients.

### 💉 [Clinical Skills](./clinical-skills/index.md)
Using AI to visualize complex anatomy (Wound Care) and conceptual metaphors (ECG Interpretation).

### 🗣️ [Communication & Empathy](./person-centred-care/index.md)
Refining person-centred language and practicing difficult conversations in a safe environment.

### 🎨 [Original Student Adaptations](./original-adaptations/index.md)
Seeing how students take ownership of AI tools to create their own visual metaphors and study aids.

## Why Case Studies?

Case studies bridge the gap between AI theory and clinical reality. They help educators:
1. **Reduce Anxiety**: See how others have successfully (and safely) used AI.
2. **Standardize Practice**: Align AI use with institutional and professional standards.
3. **Inspire Students**: Show them how to use AI for professional development, not just task completion.

---

**Explore the first category**: [Patient Scenarios](./patient-scenarios/index.md)
